...
bool Cubemap::Initialise(ID3D11Device* device, ID3D11DeviceContext* context, std::vector<Texture*> faces,
	const int width, const int height, const int mipMaps)
{
	_mipMaps = mipMaps;
	D3D11_TEXTURE2D_DESC inputDesc;
	faces[0]->GetTexture()->GetDesc(&inputDesc);
	D3D11_TEXTURE2D_DESC texDesc;
	texDesc.Width = width;
	texDesc.Height = height;
	texDesc.MipLevels = mipMaps;
	texDesc.ArraySize = 6;
	texDesc.Format = inputDesc.Format;// DXGI_FORMAT_BC1_UNORM;// DXGI_FORMAT_R16G16B16A16_FLOAT;
	texDesc.SampleDesc.Count = 1;
	texDesc.SampleDesc.Quality = 0;
	texDesc.Usage = D3D11_USAGE_DEFAULT;
	texDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
...
void Cubemap::Copy(ID3D11DeviceContext* context, std::vector<Texture*> faces, int width, int height, int mipSlice) const
{
	D3D11_BOX sourceRegion;
	for (int i = 0; i < 6; ++i)
	{
