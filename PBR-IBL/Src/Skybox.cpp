...
bool Skybox::CreateCubeMap(D3D* d3d, const HWND hwnd)
{
	...
	//bool result = image->Initialize(device, deviceContext, "environment.dds");
	const HRESULT result = image->Initialise(device, "Textures/environment.dds");
	if (!result)
	{
		return false;
	}
	...
	cubeImages.push_back(new Texture());
#if 0
	cubeImages[0]->Initialise(device, L"Env1\\negx.dds");
	cubeImages[1]->Initialise(device, L"Env1\\posx.dds");
	cubeImages[2]->Initialise(device, L"Env1\\posy.dds");
	cubeImages[3]->Initialise(device, L"Env1\\negy.dds");
	cubeImages[4]->Initialise(device, L"Env1\\posz.dds");
	cubeImages[5]->Initialise(device, L"Env1\\negz.dds");
#endif
	cubeImages[0]->Initialize(device, deviceContext, "Textures/EnvironmentCube1/negx.bmp");
	cubeImages[1]->Initialize(device, deviceContext, "Textures/EnvironmentCube1/posx.bmp");
	cubeImages[2]->Initialize(device, deviceContext, "Textures/EnvironmentCube1/posy.bmp");
	cubeImages[3]->Initialize(device, deviceContext, "Textures/EnvironmentCube1/negy.bmp");
